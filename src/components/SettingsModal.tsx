...
import { X, RotateCcw, RefreshCw, Heart, ExternalLink } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const variants: { id: ClockVariant; label: string }[] = [
    ...
    { id: 'weather-side', label: 'Weather Side' },
    { id: 'compact', label: 'Compact' },
    { id: 'large-info', label: 'Large Info' },
    { id: 'vertical-weather', label: 'Vert + Weather' }
  ];
  ...
  const renderMiniPreview = (id: ClockVariant) => {
    switch (id) {
      case 'horizontal':
        return (
          <div className="variant-preview-mini" style={{ gap: 2 }}>
            <div>
              <span className="preview-accent">1</span>9:30
            </div>
            <div className="preview-sub">Oct 26, Fri • ☁ 26°</div>
          </div>
        );
      case 'vertical':
        return (
          <div className="variant-preview-mini" style={{ gap: 2 }}>
            <div className="preview-sub">Oct 26 • ☁ 26°</div>
            <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 0.82, alignItems: 'center' }}>
              <span style={{ letterSpacing: -0.5 }}>
                <span className="preview-accent">1</span>9
              </span>
              <span>30</span>
            </div>
          </div>
      ...
      case 'day-clock':
        return (
          <div className="variant-preview-mini" style={{ gap: 1 }}>
            <div className="preview-sub">Friday</div>
            <div>
              <span className="preview-accent">1</span>9:30
            </div>
            <div className="preview-sub">Oct 26 • ☁ 26°</div>
      ...
      case 'weather-side':
        return (
          <div className="variant-preview-mini" style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            <div>
              <span className="preview-accent">1</span>9:30
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 1 }}>
              <span className="preview-sub">☁ 26°</span>
              <span className="preview-sub" style={{ fontSize: '0.55rem' }}>Oct 26, Fri</span>
            </div>
          </div>
        );
      case 'compact':
      ...
      case 'vertical-weather':
        return (
          <div className="variant-preview-mini" style={{ gap: 2 }}>
            <div className="preview-sub">Oct 26 • Cloudy ☁ 26°</div>
            <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 0.82, alignItems: 'center' }}>
              <span style={{ letterSpacing: -0.5 }}>
                <span className="preview-accent">1</span>9
              </span>
        ...
        <div className="settings-section">
          ...
          <div className="option-row">
            <div className="option-info">
              ...
              <span className="option-desc">Display time as 19:30 instead of 7:30 PM</span>
            </div>
          ...
          <div className="option-row">
            <div className="option-info">
              <span className="option-title">Show Weather</span>
              <span className="option-desc">Display temperature and weather icon</span>
            </div>
            <label className="toggle-switch">
              <input
                type="checkbox"
                checked={settings.showWeather}
                onChange={(e) => onUpdateSetting('showWeather', e.target.checked)}
              />
              <span className="toggle-slider" />
            </label>
          </div>
          ...
          {settings.showWeather && (
            <div
              ...
              style={{
                background: 'rgba(233, 40, 40, 0.06)',
                borderColor: 'rgba(233, 40, 40, 0.2)',
                display: 'flex',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div className="option-info">
                  <span className="option-title" style={{ fontSize: '0.85rem' }}>Temperature Unit</span>
                  <span className="option-desc">Select Celsius or Fahrenheit</span>
                </div>
                <div className="segmented-control" style={{ width: 120 }}>
                  <button
                    className={`segment-btn ${settings.tempUnit !== 'f' ? 'active' : ''}`}
                    onClick={() => onUpdateSetting('tempUnit', 'c')}
                  >
                    °C
                  </button>
                  <button
                    className={`segment-btn ${settings.tempUnit === 'f' ? 'active' : ''}`}
                    onClick={() => onUpdateSetting('tempUnit', 'f')}
                  >
                    °F
                  </button>
                </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 6, borderTop: '1px solid rgba(255, 255, 255, 0.08)' }}>
                <div style={{ fontSize: '0.8rem', color: 'var(--settings-subtext)' }}>
                  {weather ? (
                    <span>
                      {weather.city || 'Local'}: {weather.temperature}°{settings.tempUnit === 'f' ? 'F' : 'C'} • {weather.conditionText}
                    </span>
                  ) : (
                ...
                >
                  <RefreshCw size={12} className={isWeatherLoading ? 'spin' : ''} />
